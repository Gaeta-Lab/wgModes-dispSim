...
n1 = sellmeier('sio2',1550*10^-9);          % SiO2 lower cladding
n2 = sellmeier('sin',1550*10^-9);          % SiN core
n3 = sellmeier('hto',1550*10^-9);          % HTO upper cladding

h1 = 2000;           % lower cladding (nm)
h2 = 730;           % silicon core (nm)
h3 = 2000;           % upper cladding (nm)

dx = 2.5;           % grid size (x) (nm)
dy = dx;            % grid size (y) (nm)

lambda = 1550;      % wavelength (nm)
nmodes = 6;         % number of modes to compute

w = 1500;            % waveguide full-width (nm)
side = 1000;         % space on side of waveguide (nm)
