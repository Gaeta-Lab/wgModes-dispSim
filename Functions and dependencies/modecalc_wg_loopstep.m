...
%     hx=Hx(:,:,ii);
%     hy=Hy(:,:,ii);
%     [hz,ex,ey,ez]=postprocess(lambda0,neff_AModes(ii),hx,hy,dx_str,dy_str,eps_bend,boundary);
%     
...
% 
% plot_mode_sixfields(X,Y,AModes_fields{plot_mode_ind,1},AModes_fields{plot_mode_ind,2},'turbo');
% 
...
% 
% t_start=tic();
% [Hx,Hy,neff]=wgmodes(lambda0,guess,N_calc_SModes,dx_str,dy_str,eps_bend,boundary);
% 
...
%     hx=Hx(:,:,ii);
%     hy=Hy(:,:,ii);
%     [hz,ex,ey,ez]=postprocess(lambda0,neff_SModes(ii),hx,hy,dx_str,dy_str,eps_bend,boundary);
% 
%     % Cropping H fields to match the size of E fields
%     hx=hx(1:end-1,1:end-1);
%     hy=hy(1:end-1,1:end-1);
...
% [Phi_grid,R_grid]=cart2pol(X,Y);
% 
% plot_mode_sixfields(X,Y,SModes_fields{plot_mode_ind,1},SModes_fields{plot_mode_ind,2},'turbo');
% 
% %% Combining field
% 
